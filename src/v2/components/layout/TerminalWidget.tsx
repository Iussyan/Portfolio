...
const INITIAL_BOOT_LOGS = [
  ...
  "Connecting to secure node: 0x8F4",
  "Verifying identity...",
  "Access granted.",
  "Welcome to Iussyan Dev Shell v1.1.0",
...
export function TerminalWidget() {
  ...
  const handleCommand = useCallback((cmd: string) => {
    ...
    switch (cleanCmd) {
      case "help":
        addLog("info", (
          <div className="flex flex-col gap-1 items-start text-left">
            ...
            <span className="text-secondary">• theme - Switch between dark and light mode</span>
            <span className="text-secondary">• whoami - View professional identity</span>
            <span className="text-secondary">• ls - List directory contents</span>
            <span className="text-secondary">• archives - View project logbook</span>
            <span className="text-secondary">• achievements - View milestones</span>
            <span className="text-secondary">• contact - Get in touch</span>
            <span className="text-secondary">• sysinfo - System telemetry</span>
            <span className="text-secondary">• override core - Toggle system architecture (v1/v2)</span>
            <span className="text-secondary">• reboot - Restart the shell</span>
            <span className="text-secondary">• clear - Clear terminal buffer</span>
            <span className="text-secondary">• close - Minimize terminal</span>
          </div>
        ));
        break;
      case "theme":
      case "themes":
        addLog("info", (
          <div className="flex flex-col gap-2">
            <span className="text-primary font-bold uppercase tracking-widest">[Appearance Settings]</span>
            <div className="grid grid-cols-1 gap-1 px-2 border-l border-primary/20 ml-2">
              <span className="text-secondary">• dark  - Glacier Dark (Professional)</span>
              <span className="text-secondary">• light - Glacier Light (Editorial)</span>
            </div>
      ...
      case "theme set light":
        {
          ...
          setTheme(mode);
          addLog("info", `Updating display mode focus: ${mode.toUpperCase()}`);
      ...
      case "terminal":
        ...
        break;
      case "override core":
        addLog("info", "INITIATING_CORE_RESTRUCTURE...");
        addLog("info", "SWAPPING_SOURCE_LIBRARIES...");
        tacticalAudio?.hum();
        toggleVersion();
        break;
      ...
      case "override":
        {
          ...
          setTheme(nextTheme);
          addLog("info", `OVERRIDE_ACTIVE: TOGGLING_VISUAL_MODE... NEW_STATE: ${nextTheme.toUpperCase()}`);
          tacticalAudio?.click();
        }
        break;
      case "classified":
  ...
  }, [addLog, router, theme, setTheme, isHacked, isFullscreen, achievements]);
          ...
          >
            ...
            <div className="border-b border-primary/20 p-2 sm:p-3 flex justify-between items-center text-[10px] font-bold tracking-widest text-primary/60 bg-surface-medium shrink-0">
              <div className="flex items-center gap-3 pl-1">
                ...
                <span className="hidden sm:inline">Iussyan Dev Shell // v1.1.0</span>
                <span className="sm:hidden text-primary animate-pulse">Active Session</span>
              </div>
              ...
              {logs.map((log, i) => (
                <div key={i} className={cn(
                  ...
                  isHacked ? "text-emerald-500 font-bold drop-shadow-[0_0_8px_rgba(16,185,129,0.8)]" :
                    log.type === "command" ? "text-secondary font-bold" :
                      log.type === "error" ? "text-tertiary" :
                        log.type === "info" ? "text-primary/60" :
                          "text-primary"
                )}>
                  <span className={cn(
                    "shrink-0 select-none hidden sm:block opacity-20"
                  )}>[{log.timestamp}]</span>
            ...
            <div className="border-t border-primary/10 p-2 bg-surface-low text-[9px] font-bold text-cyan-400 uppercase tracking-tighter flex justify-between items-center px-4 shrink-0">
              <div className="flex items-center gap-4">
                ...
                <span className="text-secondary">Core: {version.toUpperCase()} [ACTIVE]</span>
              </div>
              <div className="flex items-center gap-4">
                <span>Buffer: {logs.length}</span>
                <span className="text-secondary">Connection: Secure</span>
              </div>
            </div>
          </motion.div>
        )}
