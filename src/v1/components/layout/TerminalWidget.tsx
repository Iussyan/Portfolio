...
export function TerminalWidget() {
  ...
  const { achievements, unlockAchievement } = useAchievement();
  ...
  const handleCommand = useCallback((cmd: string) => {
    ...
    switch (cleanCmd) {
      ...
      case "system_restore":
        {
          const nextTheme = theme === "tactical" ? "professional" : "tactical";
          toggleTheme();
          addLog("info", `INITIATING_SYSTEM_SHIFT_TO: ${nextTheme.toUpperCase()}`);
          tacticalAudio?.glitch();
          ...
          >
            ...
            <div className="border-t border-primary/10 p-2 bg-surface-low text-[9px] font-bold text-cyan-400 uppercase tracking-tighter flex justify-between items-center px-4 shrink-0">
               <div className="flex items-center gap-4">
                ...
                <span className="text-secondary font-mono tracking-widest text-[8px]">CORE: {version.toUpperCase()} [ACTIVE]</span>
              </div>
              <div className="flex items-center gap-4">
